...
* `Ctrl-XR`, `Meta-R`   : カーソルで選択したヒストリを挿入する (by `box.lua`)
* `Ctrl-XG`, `Meta-G`   : カーソルで選択した Git Revision を挿入する (by `box.lua`)
* `Ctrl-XH`, `Meta-H`   : カーソルで選択した過去に移動したディレクトリを挿入する (by `box.lua`)
* `Ctrl-Q`, `Ctrl-V`    : タイプした文字をそのまま挿入する
* `Ctrl-Right`, `Meta-F`: 次の単語先頭へ移動
* `Ctrl-Left`, `Meta-B` : 前の単語先頭へ移動
* `Ctrl-_`, `Ctrl-Z`    : 直前の変更を取り消す
* `Meta-O`              : ショートカットのパスをリンク先のファイル名に置換
