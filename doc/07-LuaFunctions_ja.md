...
コマンドを実行します。外部コマンドだけでなく、nyagos 内蔵コマンドも呼び出すことができます。バッチファイルを呼び出した場合、その中で行われた環境変数の変更も呼び出し元に反映されます。

エラーが発生した時、戻り値は %ERRORLEVEL% に格納すべき整数値とエラーメッセージが入ります。エラーが無い時は (0,nil) が戻ります。

#### 形式1: `nyagos.exec("コマンドライン文字列")`

1つの文字列を **コマンドライン文字列としてそのまま実行**します。空白・引用符などの解釈は通常のシェルのコマンドラインと同じルールに従います。コマンド名や引数に空白が含まれる場合は、ユーザー自身で引用符などを適切に記述する必要があります。

例: `nyagos.exec('"C:\\Program Files\\foo\\bar.exe" arg1 arg2')`

#### 形式2: `nyagos.exec{"実行ファイル名","引数1","引数2"...}`

テーブルの各要素を **コマンド名および引数として個別に指定**します。空白を含む引数であっても自動的に適切な形で渡されます。
...
### `nyagos.eval("シェルコマンド")`
### `nyagos.evaln("シェルコマンド")`

nyagos.exec と同じですが、標準出力を取り込んで、戻り値として返します。
eval は出力末尾の CRLF を削除しますが、evaln はそのまま返します。
...
この関数は `(embed)/trash.lua` のために作られました。
