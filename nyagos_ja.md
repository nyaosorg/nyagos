...
デフォルトでは、nyagos.lua にて以下が設定されています。
...
文字列を標準出力に出力します。リダイレクトも可能ですが、文字コードは
UTF8 になります。
...
ユーザが入力したコマンドラインの内容を Lua で加工することができます。

例えば、次のような Lua コマンドを実行すると、ユーザの入力は
echo されるだけで、何も実行されなくなります。
