...
function Go-VersionInfo($version) {
    Download-Exe "github.com/josephspurrier/goversioninfo" "goversioninfo.exe"
...
function Download-Exe($url,$exename){
    ...
    go get $url
    Set-Location (Join-Path (Get-Go1stPath) "src" $url)
    go build
    Copy-Item go-bindata.exe $cwd
    Set-Location $cwd
}
...
function Build($version,$tags) {
    ...
    if( -not (Test-Path "mains\bindata.go") ){
        Download-Exe "github.com/jteeuwen/go-bindata" "go-bindata.exe"
